...
<html>
<head>
	<title><? include '../cfg/s-login-note';?> | Band decoder set</title>
...
if ( $rigctld == "0" ) {
	...
	$form1->addGroup("Band decoder daemon (read from rigctld)")
		...
		if ($form1->isSuccess()) {
			...
			$bandon = $values->pocet;
			txfile2("../cfg/s-band-on", $values->pocet);
			if ( $bandon == "0" ) {
				//exec('../script/band.sh stop > /dev/null 2>&1 &');
				exec('sudo /bin/systemctl stop bd.service');
			}
		}

	if ( rxfile2("../cfg/s-band-on") == "1" ) {	
		$form = new Form('druhy');
		$form->setMethod('POST');

		// generovani formulare
		for ($gpio = 1; $gpio < $nrgpio+1; ++$gpio){
			if ( rxfile2("../cfg/s-relay-{$gpio}") == "n/a" ) {
			  if ( $webcamon == "1" && $gpio == "16" ) {
				txfile2("../cfg/s-band{$gpio}-on", '0'); // if webcam on, gpio 16 controlled by camera
			  }else{ 
				// jmena poli
				$name = "freq".$gpio."name";
				...
				$form->addCheckbox($enable, 'Enable for band decoder')
					...
					if ($form->isSuccess()) {
						...
						txfile2("../cfg/s-band{$gpio}-on", $values->$enable);
					}
				if ( rxfile2("../cfg/s-band{$gpio}-on") == "1") {
					...
					$form->setDefaults(array(
						$name => rxfile2("../cfg/s-band{$gpio}-name"),
						$enable => rxfile2("../cfg/s-band{$gpio}-on"),
						$freqfrom => rxfile2("../cfg/s-band{$gpio}-from"),
						$freqto => rxfile2("../cfg/s-band{$gpio}-to"),
					));
				}
			  }
			}
		...
		if ($form->isSuccess()) {
			...
			for ($gpio = 1; $gpio < $nrgpio+1; ++$gpio){
				...
				$freqto = "freq".$gpio."to";
				// ulozeni promennych
				txfile2("../cfg/s-band{$gpio}-name", $values->$name);
				//txfile2("../cfg/s-band{$gpio}-on", $values->$enable);
				txfile2("../cfg/s-band{$gpio}-from", $values->$freqfrom);
				txfile2("../cfg/s-band{$gpio}-to", $values->$freqto);
			}
